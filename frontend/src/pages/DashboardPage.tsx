...
export default function DashboardPage() {
  ...
  const needsAttention = [
    ...
    ...runs.filter((r) => r.status === "failed").slice(0, 2).map((r) => `Run #${r.id.slice(0, 6)} — ${r.error_summary || "Failed"}`),
  ];

  return (
    <div>
      <h1 className="text-xl font-semibold mb-6 text-[#E8EAED]">Dashboard</h1>

      <div className="grid grid-cols-5 gap-4 mb-6">
        <Card>
          <div className="text-[#9AA0B0] text-xs mb-1 flex items-center gap-2">
            <GitBranch size={12} /> Active Workflows
          </div>
          <div className="text-2xl font-semibold text-[#E8EAED]">
        ...
        <Card>
          ...
          <div className="text-2xl font-semibold text-[#00B894]">
        ...
        <Card>
          ...
          <div className="text-2xl font-semibold text-[#FDCB6E]">
        ...
        <Card>
          ...
          <div className="text-2xl font-semibold text-[#E17055]">
            {runLoading ? "..." : failedRuns}
          </div>
        </Card>
        <Card>
          <div className="text-[#9AA0B0] text-xs mb-1 flex items-center gap-2">
          ...
          <div className="text-2xl font-semibold text-[#E8EAED]">Odoo</div>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        <Card>
          <h2 className="text-sm font-medium text-[#E8EAED] mb-3">Recent Runs</h2>
          {runLoading ? (
            ...
            <div className="space-y-2">
              {recentRuns.map((run) => (
                <div
                  ...
                  className="flex items-center justify-between py-2 px-3 rounded-md hover:bg-[#242836] cursor-pointer transition-colors"
                >
              ...
              {recentRuns.length === 0 && (
                <div className="text-[#9AA0B0] text-sm">No runs yet.</div>
              )}
            </div>
          )}
        </Card>
        ...
        <Card>
          <h2 className="text-sm font-medium text-[#E8EAED] mb-3">Workflow Templates</h2>
          <div className="space-y-2">
            {["Search & Extract", "Fill & Submit", "Open & Inspect"].map((tpl) => (
              <div
                key={tpl}
                className="py-2 px-3 rounded-md hover:bg-[#242836] cursor-pointer transition-colors text-sm text-[#9AA0B0] hover:text-[#E8EAED]"
              >
                {tpl}
              </div>
            ))}
